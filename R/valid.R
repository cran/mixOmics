...
# Note: RMSEP computation should be the same for the three modes when # 
# using PLS if same seed (for CV) or if LOOCV                         #
# --------------------------------------------------------------------#
         ...
         M = if(validation == "Mfold") 10 else nrow(X))
{
    ...
    if (missing(validation)) 
        stop("Choose between cross-validation: 'Mfold' or loo-cv: 'loo'.")
    if (missing(method)) 
        stop("Choose a method: 'pls' or 'spls'.")
    if (missing(criterion)) 
        stop("Choose a validation criterion: 'rmsep' or 'q2'.")
    if (missing(mode)) 
    ...
    if (any(criterion =='q2')) {
        if (method == 'pls') {
		    object = pls(X = X , Y = Y, ncomp = ncomp, mode = mode, 
                         max.iter = max.iter, tol = tol)
        } 
        else {
            object = spls(X = X , Y = Y, ncomp = ncomp, mode = mode, tol = tol, 
                          max.iter = max.iter, keepX = keepX, keepY = keepY)
        }
        ...
        for (h in 1:ncomp) {
	        ...
	        if (q == 1) { B.all[, , h] = W.all %*% as.vector(t(b.all[, 1:h])) } 
            else { B.all[, , h] = W.all %*% t(b.all[, 1:h]) }
             
            Y.all[, , h] = X.aux %*% B.all[, , h]
            RSS[, h] = apply((Y - Y.all[, , h])^2, 2, sum, na.rm = TRUE)  # on all data
        }  #end h
    } #end if
     
    #-- M fold cross validation or loo --#
    ##- define the folds
    if (validation == "Mfold") { fold = sample(1:M, n, replace = TRUE) } 
    else { fold = 1:n }
    ...
    for (i in 1:M) {
        ...
        c = object$mat.c
        X.test.aux = X.test
        X.test.aux[is.na(X.test)] = 0
         
        for (h in 1:ncomp) {
	        W = a[, 1:h] %*% solve(t(c[, 1:h]) %*% a[, 1:h]) 
	        if (q == 1) { B.hat[, , h] = W %*% as.vector(t(b[, 1:h])) } 
            else { B.hat[, , h] = W %*% t(b[, 1:h]) }
                          
	        Y.hat.rmsep[fold == i, , h] = X.test.aux %*% B.hat[, , h] + 
                            colMeans(as.matrix(Y.train), na.rm = TRUE)  # to be consistent with pls package
            Y.hat.press[fold == i, , h] = X.test.aux %*% B.hat[, , h]   # to be consistent with Tenenhaus
             	
            PRESS[, h] = apply((Y - Y.hat.press[, , h])^2, 2, sum, na.rm = TRUE)  # on test data
        }  #end h
    }  # end i
    ...
    if (any(criterion == "rmsep")) {
        ...
        for (j in 1:q) {
	        if(ncomp == 1) { rmsep.mat[j, ] = sqrt(mean((Y.hat.rmsep[, j, 1] - Y[, j])^2, na.rm = TRUE)) } 
            else { rmsep.mat[j, ] = sqrt(colMeans((Y.hat.rmsep[, j, 1:ncomp] - Y[, j])^2, na.rm = TRUE)) }
        }
         
        intercept = sqrt(mean((Y[, 1])^2, na.rm = TRUE))
    ...
    #-- compute q2 --#
    if (any(criterion == "q2")) {
        if (q == 1) { RSS.0 = c(rep(n - 1, q), RSS[, -ncomp]) }
        else { RSS.0 = cbind(rep(n - 1, q), RSS[, -ncomp]) }
         
        #-- q2 = 1 - PRESS/RSS --#
        if (q == 1) { q2 = 1 - PRESS/RSS.0 }
        else{q2 = 1 - apply(PRESS, 2, sum) / apply(RSS.0, 2, sum) }
        ...
        if (q == 1) { colnames(q2) = colnames(q2V) = paste("comp", 1:ncomp) }
        else { names(q2) = colnames(q2V) = paste("comp", 1:ncomp) }
    }  #end if
    ...
    return(invisible(list(
            ...
            Q2 = if (any(criterion == 'q2')) { 
                    list(RSS = RSS, PRESS = PRESS, q2 = q2, q2V = q2V) 
                } 
                 else NULL 
        )))
}
